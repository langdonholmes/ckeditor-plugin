...
export default class HLJSCodeBlockEditing extends Plugin {
  ...
  init() {
    ...
    editor.commands.add('indentHLJSCodeBlock', new IndentHLJSCodeBlockCommand(editor));
    editor.commands.add('outdentHLJSCodeBlock', new OutdentHLJSCodeBlockCommand(editor));
    ...
    editor.keystrokes.set('Shift+Tab', getCommandExecuter('outdentHLJSCodeBlock'));
    ...
    console.log(normalizedLanguagesDefs)
    // Conversion.
    editor.editing.downcastDispatcher.on('insert:hljsCodeBlock', modelToViewCodeBlockInsertion(model, normalizedLanguagesDefs, true));
    editor.data.downcastDispatcher.on('insert:hljsCodeBlock', modelToViewCodeBlockInsertion(model, normalizedLanguagesDefs));
    ...
    this.listenTo(model, 'getSelectedContent', (evt, [selection]) => {
      ...
      if (selection.isCollapsed || !anchor.parent.is('element', 'hljsCodeBlock') || !anchor.hasSameParentAs(selection.focus)) {
        return;
      }
      ...
      model.change(writer => {
        ...
        if (docFragment.childCount > 1 || selection.containsEntireContent(anchor.parent)) {
          const codeBlock = writer.createElement('hljsCodeBlock', anchor.parent.getAttributes());
