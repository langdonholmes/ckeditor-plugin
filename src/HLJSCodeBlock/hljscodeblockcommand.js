...
export default class HLJSCodeBlockCommand extends Command {
	...
	execute( options = {} ) {
		...
		console.log("normalized languages: ")
	...
	_getValue() {
		...
		const isCodeBlock = !!( firstBlock && firstBlock.is( 'element', 'hljsCodeBlock' ) );
	...
	_removeCodeBlock( writer, blocks ) {
		const codeBlocks = blocks.filter( block => block.is( 'element', 'hsjsCodeBlock' ) );

		for ( const block of codeBlocks ) {
			const range = writer.createRangeOn( block );

			for ( const item of Array.from( range.getItems() ).reverse() ) {
				if ( item.is( 'element', 'softBreak' ) && item.parent.is( 'element', 'hsjsCodeBlock' ) ) {
